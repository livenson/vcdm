...
import vcdm
CLOUD_ACCOUNT = vcdm.config.get('azure', 'credentials.account')
CLOUD_SECRET_KEY = vcdm.config.get('azure', 'credentials.password')

CLOUD_BLOB_HOST = vcdm.config.get('azure', 'credentials.blob_url')
CLOUD_TABLE_HOST = vcdm.config.get('azure', 'credentials.table_url')
CLOUD_QUEUE_HOST = vcdm.config.get('azure', 'credentials.queue_url')
...
class QueueStorage(Storage):
    def __init__(self, host = CLOUD_TABLE_HOST, account_name = CLOUD_ACCOUNT, secret_key = CLOUD_SECRET_KEY, use_path_style_uris = None):
...
class BlobStorage(Storage):
    def __init__(self, host = CLOUD_BLOB_HOST, account_name = CLOUD_ACCOUNT, secret_key = CLOUD_SECRET_KEY, use_path_style_uris = None):
